...
public interface GoodyRepository extends JpaRepository<Goody, String> {
    ...
    @Query(value = "SELECT new io.xhub.xquiz.domain.projection.ParticipantGoodiesDTO(" +
            ...
            "                                                g.label, " +
            "                                                g.availableStock, " +
            "                                                case when ((qid.quizInstance.percentFinalScore * :techQuizFactor) / 100)  +  ((cqd.percentFinalScore * :cultQuizFactor) / 100)  >= g.winScoreThreshold then true else false end, " +
            ...
            "       GROUP BY g.id, qid.quizInstance.percentFinalScore, " +
